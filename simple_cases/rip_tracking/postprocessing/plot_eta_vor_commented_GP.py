...
fdir = "../../../../simulationRuns/rip_tracking/output/"
...
def executeSubplot(i):
    ...
    # Adding the jet colormap to the subplot
    hp = plt.pcolormesh(xx, yy, eta, cmap = jet)

    # Framing the subplot's axes
    plt.axis([0, m * dx / 2, 0, n * dy])

    # Adding the colorbar to the subplot
    h_bar = plt.colorbar(hp, orientation = "horizontal")

    # Applying the MaxNLocator to the colorbar
    tick_locator = ticker.MaxNLocator(nbins = 5)
    h_bar.locator = tick_locator
    h_bar.update_ticks()

    # Labeling the colorbar's x-axis to measure U in meters
    h_bar.ax.set_xlabel(r'$\eta (m)$')

    # Labeling the subplot's axes to measure its dimensions in meters
    plt.xlabel('x (m)')
    plt.ylabel('y (m)')
    ...
    # Adding the contours in the subplot
    cf = plt.contourf(xx, yy, -dep, 10)

    # Adding the colorbar to the subplot and labeling its x-axis to measure depth in meters
    c_bar = plt.colorbar(cf, orientation = 'horizontal')
    c_bar.ax.set_xlabel('depth (m)')

    # Adding quivers detailing <FIXME> to the subplot
    q = plt.quiver( xx[ ::EVERYWHICH, ::EVERYWHICH],
                    yy[ ::EVERYWHICH, ::EVERYWHICH],
                    u[  ::EVERYWHICH, ::EVERYWHICH],
                    v[  ::EVERYWHICH, ::EVERYWHICH])
    
    # Labeling the subplot's x axis to measure it's x dimension in meters
    plt.xlabel('x (m)')
