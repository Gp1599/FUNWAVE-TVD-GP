...
class ETAPlot:
    ...
    def __init__(self, fileNumber):
        ...
        self.y = readETAData(fileNumber, numOfETA, outputDir)
        self.x = np.linspace(0, Lt, Mglob)
        # end of reference
    ...
    def printResults(self):
        ...
        plt.plot(self.x, self.y[1, :], 'c-', linewidth = 0.2)
        ...
        # Water Fill
        plt.fill_between(self.x, depth[1, :], self.y[1, :], where = self.y[1, :] > depth[1, :], facecolor = 'cyan', interpolate = True)
        
        # Bottom Fill
        plt.fill_between(self.x, min(depth[1, :]) - .05, depth[1, :], where = depth[1, :] > (depth[1, :] - .05), facecolor = '0.35', hatch = 'X')
        ...
        # Finally, print the snapshot containing the final results
        snapshotFileName = plotsDir + "/surf_{0:05d}.png".format(self.fileNumber)
        plt.savefig(snapshotFileName, ext = "png", bbox_inches = "tight")

        plt.close()
        # end of reference
# end of Gabriel's work

# Plot and save every ETA file:
for i in range(1, numOfETA+1):
    p = ETAPlot(i)                              #Gabriel's
    p.printResults()                            #Gabriel's
